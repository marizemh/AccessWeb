...
export class ScrumboardService {
	...
	deleteCard(id: string): Observable<boolean> {
		return this.board$.pipe(
			take(1),
			switchMap((board) =>
				this._httpClient.delete('api/apps/scrumboard/board/card', { params: { id } }).pipe(
					map((isDeleted: boolean) => {
	 ...
	 * @param cards
	 */
	updateCardPositions(cards: Card[]): void {
		// Observable<Card[]>
	...
	deleteLabel(id: string): Observable<boolean> {
		return this.board$.pipe(
			take(1),
			switchMap((board) =>
				this._httpClient.delete('api/apps/scrumboard/board/label', { params: { id } }).pipe(
					map((isDeleted: boolean) => {
